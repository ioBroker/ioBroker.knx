            ...
            parser.parseString(xml0, function (err, result) {
                ...
                for (var i = 0; i < matchesResTmp.length; i++){
                    if (matchesResTmp[i].Connectors) {
                       // if (matchesResTmp[i].Connectors[0].hasOwnProperty('$')){
...
//                            for (var iy = 0; iy < matchesRes.length; iy++)
                           // var iy = matchesRes.map(function (img) { return img.$.Description; }).indexOf(ug.$.Name);
                            var iy = matchesRes.map(function (img) {
                                    return img.Connectors[0].Send[0].$.GroupAddressRefId; }).indexOf(ug.$.Id);
                            if (iy != -1)
                            {
                                {
                                    if (matchesRes[iy].$.hasOwnProperty('DatapointType')){
                                        dpt = matchesRes[iy].$.DatapointType;
                                    } else {
                                        dpt = matchesRes[1].$.DatapointType;
                                    }
