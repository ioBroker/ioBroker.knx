...
//var deviceComObjectRefId = {};

var propertiesOfGroupAddress = {};

module.exports = {
/*
    getDeviceProperties: function (fileName, deviceFiles, callback) {

        if (typeof deviceFiles === 'function') {
            callback = deviceFiles;
            deviceFiles = null;
        }

        function processFile(err, deviceFiles, xml0 ) {
            ...
            parser.parseString(xml0, function (err, result) {
                var doc = new dom().parseFromString(deviceFiles);


            });
        }
        if (typeof deviceFiles === 'string' && typeof fileName === 'string') {
            processFile(null, deviceFiles, fileName);
        } else {
    ...
    getGAS: function (fileName, knx_master, callback) {
        ...
        function processFile(err, knxMaster, xml0 ) {
            ...
            parser.parseString(xml0, function (err, result) {
                ...
                var rePattern = new RegExp(/stat(e|us)|rm|r\/:*/);
                ...
                _.each(select.select("//*[local-name(.)='Connectors']/*/@GroupAddressRefId", doc), function (grpAddressRefId) {
                    ...
                    if (grpAddressRefId.ownerElement.parentNode.parentNode.getAttribute('ReadFlag'))
                        read = grpAddressRefId.ownerElement.parentNode.parentNode.getAttribute('ReadFlag');

                    if (grpAddressRefId.ownerElement.parentNode.parentNode.getAttribute('WriteFlag'))
                        write = grpAddressRefId.ownerElement.parentNode.parentNode.getAttribute('WriteFlag');

                    if (grpAddressRefId.ownerElement.parentNode.parentNode.getAttribute('UpdateFlag'))
                        update = read; //grpAddressRefId.ownerElement.parentNode.parentNode.getAttribute('UpdateFlag');
    ...
    if (fs.existsSync(knxProjFileName)) {
        ...
        zipEntries.forEach(function (zipEntry) {
            ...
            if (zipEntry.name === '0.xml') {
                ...
                console.log('Extracting : ' + zipEntry.name); // outputs zip entries information
                if (knxMaster !== undefined && xml0 !== undefined && deviceFiles !== undefined) {
                    callback(null, knxMaster, xml0, deviceFiles)
                }
            }
/*
            if ( zipEntry.name.match(/\/M.*\.xml$/) ){
                deviceFiles = zip.readAsText(zipEntry.entryName) || '';
