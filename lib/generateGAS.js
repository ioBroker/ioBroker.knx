...
var statusGroupAddress = {};
var readFlagOfgroupAddressRefId = {};
var writeFlagOfgroupAddressRefId = {};
var updateFlagOfgroupAddressRefId = {};
    ...
    getGAS : function(fileName, knx_master, callback) {
        ...
        function processFile(err, knxMaster, xml0 ) {
            ...
            parser.parseString(xml0, function (err, result) {
                ...
                // map DatapointTypes => GroupAddressRefId
                _.each( select.select("//*[local-name(.)='Send']", doc), function(dpt) {
                   var gaId = dpt.getAttribute('GroupAddressRefId');
                   dptOfgroupAddress[gaId] = dpt.parentNode.parentNode.getAttribute('DatapointType');
                    readFlagOfgroupAddressRefId[gaId] = 'Enabled';
                    writeFlagOfgroupAddressRefId[gaId] = 'Disabled';
                    updateFlagOfgroupAddressRefId[gaId] = 'Enabled';
                });
...
// find control+status pairs
                ...
                _.each(statusGaNames, function(statusga, statusganame) {
                    ...
                    if (similarity(statusgabasename, similars.toLowerCase()) > 0.7) {
                        statusGroupAddress[gaNames[similars]] = statusga;
                        readFlagOfgroupAddressRefId[gaNames[similars]] = 'Disabled';
                        writeFlagOfgroupAddressRefId[gaNames[similars]] = 'Enabled';
                        updateFlagOfgroupAddressRefId[gaNames[similars]] = 'Disabled';
                        readFlagOfgroupAddressRefId[statusga] = 'Enabled';
                        writeFlagOfgroupAddressRefId[statusga] = 'Disabled';
                        updateFlagOfgroupAddressRefId[statusga] = 'Enabled';

               //         console.log("*** possible match: (%s %s) \t <==> (%s %s)",
               //             adr2ga(groupAddresses[statusga].getAttribute('Address')), statusganame,
               //             adr2ga(groupAddresses[gaNames[similars]].getAttribute('Address')), similars);
                    }
                });
                ...
                _.each ( groupAddresses , function (groupAddress) {
                    ...
                    var gaName = ga.getAttribute('Name');
                    var dpt = "";
                    if (id in dptOfgroupAddress)
                        dpt = dptOfgroupAddress[id];
                    ...
                    // roletmp muss generiert werden

                    var values = setValues( readFlagOfgroupAddressRefId[id],
                                            writeFlagOfgroupAddressRefId[id],
                                            updateFlagOfgroupAddressRefId[id],
                                            dpt,
                                            dptObj,
                                            dpstObj);
                    ...
                    tmpobj = {
                        ...
                        common: {
                            ...
                            read: changeValue(readFlagOfgroupAddressRefId[id]),   // mandatory, default true
                            write: changeValue(writeFlagOfgroupAddressRefId[id]),  // mandatory, default false
                        ...
                        native: {
                            ...
                            addressRefId: id,
                            //statusGARefId: statusGroupAddress(id)
                        }
                    };
                    ...
                    _.each( select.select("./*[local-name(.)='DeviceInstanceRef']/@RefId", roomnode), function(deviceRefId) {
                        ...
                        if (deviceComObjects.hasOwnProperty(deviceRefId.value)) {
                            _.each( select.select("./*[local-name(.)='ComObjectInstanceRef']/*[local-name(.)='Connectors']/*[local-name(.)='Send']/@GroupAddressRefId", deviceComObjects[deviceRefId.value]), function(gaRef) {
                                ...
                                var ga = gaRef.value;
                                if (!(ugNameList === '')) {
                                    //ugNameList = ugNameList + ',' + ga.getAttribute('Name');
